...
export const SellerModel = {
  async list() {
    const result = await pool.query("SELECT id, nome, email, ativo FROM sellers ORDER BY nome ASC");
    return result.rows;
  },

  async get(id) {
    const result = await pool.query("SELECT id, nome, email, ativo FROM sellers WHERE id = $1", [id]);
